...
def find_best_autoencoders(batchwise_data, autoencoders):
    with torch.no_grad():
        ...
        for index, autoencoder in autoencoders.items():
            ...
            #print(f"reconError for index {index}: {reconstruction_error_autoencoder}, {reconstruction_error_data}, {relatedness}")
...
def find_num_of_outliers(batchwise_data, model):
    with torch.no_grad():
        ...
        #print(element_mean,  model.mean + 3 * model.std)
        outliers = sum([item >= model.mean + 3 * model.std for item in element_mean]).item()
        return outliers
