...
class PermutedAndSplittedMNIST(datasets.MNIST):

    def __init__(self, root="./MNIST/", train=True, permute_idx=None, class_indices=None, task_num=None):
        super(PermutedAndSplittedMNIST, self).__init__(root, train, download=True)
        ...
        if self.train:
            ...
            self.data_train = torch.stack([img.float().view(-1)[permute_idx] /255 for img in subset])
            ...
            self.data_test = torch.stack([img.float().view(-1)[permute_idx] /255 for img in subset])
